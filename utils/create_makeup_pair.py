...
TODO: align face before editing
"""
...
def create_makeup_pair(args, img_id_path_list):
    ...
    prompt_base = "Add makeup to the person. The makeup is {} Preserve the original identity, facial features, eye color, expression, hairstyle, pose and skin texture. Maintain the original photographic quality, lighting, composition, background, camera angle, framing, and perspective."
        ...
        for makeup_idx, (category, style, desp) in enumerate(makeup_info_subset):
            ...
            for img_idx in range(args.num_img_makeup):
                ...
                if args.edit_method == "openai":
                    ...
                    img_makeup = sample_gpt_edit(client, img_id_path, mask_alpha_path, prompt, model="gpt-5")
                elif args.edit_method == "kontext":
...
def sample_celeb_id(args):
    ...
    for celeb_idx, (profession, celeb) in enumerate(celeb_info):
        ...
        for img_idx in range(args.num_img_no_makeup):
            ...
            if args.t2i_method == "openai":
                img_id = sample_gpt_t2i(client, prompt_no_makeup, model="gpt-5")
            elif args.t2i_method == "flux2":
...
class FaceIDFilter():
    def __init__(self, mode="gpt"):
        ...
        ---
        # Task 2: Evaluate similarity of facial identity and expression
        1. Assess the similarity of facial identity and facial expression between the two face images.
        2. Consider facial identity and expression, ignore makeup, hairstyle and background.
        3. Identity score: Identity similarity
            - Focus on facial structure (jawline, cheekbones, nose shape), eye shape and eye color, and relative facial proportions
            - Provide a score on a scale from 0 to 10:
        ...
        4. Expression score: Expression similarity
        ...
        5. Output the two scores in a list: [Identity score, Expression score].
        ---
        ...
        "similarity": [Identity score, Expression score],
        ...
        json_schema = {
            ...
            "required": ["image_quality", "similarity", "reasoning"],
    ...
    def __call__(self, img_id, img_makeup, img_size=256):
        ...
        while not success:
            try:
                ...
                # Remove optional fenced code block
                ...
                if not isinstance(similarity, list) or len(similarity) != 2:
                    similarity = [0, 0]

                quality = int(quality or 0)
                similarity = [int(similarity[0] or 0), int(similarity[1] or 0)]
        ...
        if quality == 1 and similarity[0] >= 9 and similarity[1] >= 8:
            return False
        else:
