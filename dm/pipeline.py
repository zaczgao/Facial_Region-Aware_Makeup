...
from diffusers.models import ControlNetModel, MultiControlNetModel, MultiControlNetUnionModel
...
from diffusers.utils.torch_utils import is_compiled_module, is_torch_version, randn_tensor, empty_device_cache
...
class MakeupSDPipeline(StableDiffusionControlNetInpaintPipeline):
    def init_extra(self, style_clip, makeup_adapter, placeholder_token_ids, clip_hidden):
        self.placeholder_token_ids = placeholder_token_ids
        self.clip_hidden = clip_hidden

        self.unet.eval()
        self.style_clip = style_clip.eval()
        self.makeup_adapter = makeup_adapter.eval()

    # instantid
    def set_ip_adapter_scale(self, scale):
        unet = getattr(self, self.unet_name) if not hasattr(self, "unet") else self.unet
        for attn_processor in unet.attn_processors.values():
            if isinstance(attn_processor, CustomCrossAttnProcessor):
    ...
    def _encode_prompt_image_emb(self, prompt_image_emb, device, num_images_per_prompt, dtype,
        ...
        if prompt_image_emb.ndim != 3:
            prompt_image_emb = prompt_image_emb.unsqueeze(dim=0)
        ...
        prompt_image_emb = self.makeup_adapter.style_proj(prompt_image_emb)
    ...
    #     if do_classifier_free_guidance:
    #         prompt_image_emb = torch.cat([torch.zeros_like(prompt_image_emb), prompt_image_emb], dim=0)
    #         # prompt_image_emb = torch.cat([prompt_image_emb, prompt_image_emb], dim=0)
    #
    #     bs_embed, seq_len, _ = prompt_image_emb.shape
    #     prompt_image_emb = prompt_image_emb.repeat(1, num_images_per_prompt, 1)
    #     prompt_image_emb = prompt_image_emb.view(bs_embed * num_images_per_prompt, seq_len, -1)
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            untruncated_ids = self.tokenizer(prompt, replace_token=replace_token, token_idx=token_idx,
            ...
            modified_hs = None
            if self.makeup_adapter.use_text_inv:
                style_emb = self.makeup_adapter.style_proj(style_feat)
                modified_hs = self.text_encoder.text_model.forward_embeddings_with_mapper(
                    text_input_ids.to(device), None, style_emb, self.placeholder_token_ids
                )
    ...
    def __call__(
        ...
        if not isinstance(control_guidance_start, list) and isinstance(control_guidance_end, list):
            ...
            if isinstance(controlnet, ControlNetModel) or isinstance(controlnet, MultiControlNetModel):
        ...
        elif isinstance(controlnet_conditioning_scale, float):
            mult = len(controlnet.nets) if isinstance(controlnet, MultiControlNetUnionModel) else len(control_mode)
        ...
        global_pool_conditions = (
            ...
            if isinstance(controlnet, ControlNetModel) or isinstance(controlnet, ControlNetUnionModel)
        ...
        if self.makeup_adapter.use_ipa:
            prompt_image_emb_list = []
            for style_feat in style_feat_list:
                prompt_image_emb = self._encode_prompt_image_emb(style_feat,
                                                                 device,
                                                                 num_images_per_prompt,
                                                                 self.unet.dtype,
                                                                 self.do_classifier_free_guidance)
                prompt_image_emb_list.append(prompt_image_emb)
        ...
        elif isinstance(controlnet, ControlNetUnionModel):
            control_images = []
        ...
        null_text_embeds = self.text_encoder(null_text_input_ids.to(device))[0]
