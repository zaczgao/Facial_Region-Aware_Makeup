"""
diffusers-main\examples\research_projects\multi_token_textual_inversion\multi_token_clip.py
...
class MultiTokenCLIPTokenizer(CLIPTokenizer):
    ...
    def replace_placeholder_tokens_in_text(self, text, vector_shuffle=False, prop_tokens_to_load=1.0, token_idx=None):
        """
    ...
    def __call__(self, text, *args, vector_shuffle=False, prop_tokens_to_load=1.0, replace_token=True, token_idx=None, **kwargs):
        if replace_token:
            text = self.replace_placeholder_tokens_in_text(
                text, vector_shuffle=vector_shuffle, prop_tokens_to_load=prop_tokens_to_load, token_idx=token_idx
            )
    ...
    def encode(self, text, *args, vector_shuffle=False, prop_tokens_to_load=1.0, replace_token=True, token_idx=None, **kwargs):
        if replace_token:
            text = self.replace_placeholder_tokens_in_text(
                text, vector_shuffle=vector_shuffle, prop_tokens_to_load=prop_tokens_to_load, token_idx=token_idx
            )
