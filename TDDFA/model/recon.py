...
class face_model:
    def __init__(self, backbone='resnet50', device='cuda', use_ldm68=True, use_ldm106=False, use_ldm134=False,
                 use_seg=False, use_seg_visible=False, extractTex=False):

        self.backbone = backbone
        self.use_ldm68 = use_ldm68
        self.use_ldm106 = use_ldm106
        self.use_ldm134 = use_ldm134
        ...
        model = np.load(os.path.join(SCRIPT_DIR, "../assets/face_model.npy"), allow_pickle=True).item()
        ...
        if self.backbone == 'resnet50':
            ...
            self.net_recon.load_state_dict(torch.load(os.path.join(SCRIPT_DIR, "../assets/net_recon.pth"), map_location=torch.device('cpu'))['net_recon'])
            self.net_recon = self.net_recon.to(self.device)
            self.net_recon.eval()

        if self.backbone == 'mbnetv3':
    ...
    def forward(self, input_img, no_pca = False, alpha_dict_src=None):
        ...
        if alpha_dict_src is not None:
            ...
            alpha_dict['trans'] = alpha_dict_src['trans'].clone()
        ...
        if self.use_seg_visible or self.extractTex:
        ...
        if self.extractTex:
            ...
            img_colors = bilinear_interpolate(input_img.permute(0, 2, 3, 1).detach()[0], v2d[0, :, 0].detach(), 223 - v2d[0, :, 1].detach())
